...
const express = require("express"); // Framework web léger pour Node.js
...
const app = express();
// ===== MIDDLEWARES =====
app.use(cors()); // Autoriser toutes les requêtes provenant d'autres domaines
app.use(bodyParser.json()); // Parse le corps des requêtes JSON
app.use(bodyParser.urlencoded({
    extended: true
})); // Parse les formulaires (application/x-www-formurlencoded)
// ===== SERVIR LES FICHIERS STATIQUES =====
app.use(express.static(path.join(__dirname, "public"))); // Permet d'accéder à public/index.html
// ===== CONNEXION À SQLITE =====
const db = new sqlite3.Database("carrefour.sqlite", (err) => {
    if (err)
    ...
    db.get(
        ...
        (err, row) => {
            if (err) {
                ...
                return res.json({
                    message: "Erreur SQLite : " + err.message
                });
            }
            if (!row) {
